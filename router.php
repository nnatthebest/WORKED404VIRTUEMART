...
//    echo '<pre>';
//    var_dump($vars['virtuemart_category_id']);
//    var_dump(!isset($vars['virtuemart_category_id']));
//    echo '</pre>';
//    die();
//    echo '<pre>';
//    var_dump($vars['view']);
...
//    die();
	...
	if (!isset($vars['virtuemart_category_id'])){
	    ...
	    if ($vars['view'] == 'productdetails'){

        }else{
            header('HTTP/1.1 404 Not Found'); JError::raiseError(404, JText::_("Page Not Found"));
        }
	...
	public function getCategoryRouteNocache($catId,$manId){
		...
		if (! array_key_exists ($catId . 'mf' . $manId . VmConfig::$vmlang, self::$_catRoute)){
			...
			if ($ismenu==false) {
				...
				if (!isset ($this->CategoryName[$catId])) {
					$this->CategoryName[$catId] = $this->getCategoryNames($catId, $menuCatid );
				}
				$category->route .= $this->CategoryName[$catId] ;
